...
public class DashRingSpawner : MonoBehaviour
{
	...
	[SerializeField] private DashVFXBehavior[] dashVFXPrefabs;
	[SerializeField] private DashVFXBehavior currentDash;
	...
	private void OnDashFinished(Vector3 start, Vector3 end)
	{
		...
		// ---------- Anger @ START ----------
		{
			...
			{
				currentDash = GetDashVFX("Anger Dash AOE_VFX");
		...
		// ---------- Joy @ START ----------
		{
			...
			{
				currentDash = GetDashVFX("Joy Dash AOE_VFX");
		...
		// ---------- Sadness @ START (single spawn) ----------
		{
			...
			{
				currentDash = GetDashVFX("Sadness Dash AOE_VFX");
		...
		// ---------- Love @ START ----------
		{
			...
			{
				currentDash = GetDashVFX("Love Dash AOE_VFX");
		...
		// ---------- Fear @ START (single instance globally) ----------
		{
			...
			{
				currentDash = GetDashVFX("Fear Dash AOE_VFX");
		...
		if (currentDash)
		{
			var vfxInstance = Instantiate(currentDash, start, Quaternion.identity);
			vfxInstance.Spawn(vfxLifetime);
			if (debugLogs) Debug.Log($"[DashRingSpawner] [Ring] Spawned @ {start}", currentDash);
		}
	}

	private static int GetLvl(int raw) => Mathf.Clamp(raw, 0, 3);

	private DashVFXBehavior GetDashVFX(string name)
    {
		foreach (var dash in dashVFXPrefabs)
		{
			if (dash.name == name)
			{
				return dash;
